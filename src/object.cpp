...
std::string f_param::str() { return object::o_type_str(type) + " " + symbol; }
...
bool obj_equals::operator()(object *o1, object *o2) const {
  ...
  switch (o1->type) {
  case o_num: {
    // std::cout << std::to_string( std::get<double>(o1->store) ==
    // std::get<double>(o2->store));
...
object::object() { type = o_none; }

object::object(o_type _type) { type = _type; }

void object::set_params(std::vector<f_param> &_f_params) {
  f_params = _f_params;
}

void object::set_body(ast_node *_f_body) { f_body = _f_body; }
...
std::string object::str() {
  ...
  case o_num: {
    return is_int(false) ? std::to_string((int)std::get<double>(store))
                         : std::to_string(std::get<double>(store));
  }
  case o_bool: {
  ...
  if (positive)
    return std::holds_alternative<double>(store) &&
           std::get<double>(store) ==
               static_cast<int>(std::get<double>(store)) &&
...
object *object::contains(object *o) {
  ...
  case o_str: {
    ...
    std::size_t counter =
        std::get<std::string>(store).find(std::get<std::string>(o->store));
  ...
  case o_arr: {
    ...
    for (int i = 0; i < std::get<std::vector<object *>>(store).size(); i++) {
      bool same = std::get<bool>(
          ((std::get<std::vector<object *>>(store)[i])->equals(o))->store);
      if (same) {
        r_ind = i;
  ...
  case o_set: {
    object *ret = new object(o_bool);
    // std::cout << (std::get<std::unordered_set<object*, obj_hash,
    // obj_equals>>(store)).find(o) != std::get<std::unordered_set<object*,
    // obj_hash, obj_equals>>(store).end();
    ret->set(
        (std::get<std::unordered_set<object *, obj_hash, obj_equals>>(store))
            .find(o) !=
        std::get<std::unordered_set<object *, obj_hash, obj_equals>>(store)
...
object *object::reverse() {
  switch (type) {
  case o_str: {
    std::reverse(std::get<std::string>(store).begin(),
...
object *object::sort() {
  ...
  case o_arr: {
    std::sort(std::get<std::vector<object *>>(store).begin(),
              std::get<std::vector<object *>>(store).end(), compare_obj);
    break;
  }
  default:
...
object *object::fill(object *start, object *end, object *o) {
  if (!start->is_int(true) || !end->is_int(true))
    err("fill must be called with unsigned integer parameters");
  int start_ind = std::get<double>(start->store);
  int max_ind = std::get<double>(end->store);
  switch (type) {
  case o_arr: {
    switch (o->type) {
    case o_str: {
      while (start_ind < max_ind) {
        object *ret = new object(o_str);
        ret->set(std::get<std::string>(o->store));
        std::get<std::vector<object *>>(store)[start_ind] = ret;
        ++start_ind;
      }
      break;
    }
    case o_num: {
      ...
      while (start_ind < max_ind) {
        object *ret = new object(o_num);
        ret->set(std::get<double>(o->store));
        std::get<std::vector<object *>>(store)[start_ind] = ret;
        ++start_ind;
      }
      break;
    ...
    default: {
      err("cannot copy non integer and string types");
      break; }
    }
    break;
  }
  default: {
    err("fill() not supported for this type");
    break;
  }
  }
  return new object();
...
object *object::at(object *index) {
  ...
  case o_map: {
    if (std::get<std::unordered_map<object *, object *, obj_hash, obj_equals>>(
            store)
            .find(index) !=
        std::get<std::unordered_map<object *, object *, obj_hash, obj_equals>>(
            ...
            .end())
      return std::get<
          std::unordered_map<object *, object *, obj_hash, obj_equals>>(
          store)[index];
    else {
...
object *object::sub(object *start, object *end, object *step) {
  ...
  case o_str: {
    object *ret = new object(o_str);
    std::string tmp;
    for (int i = std::get<double>(start->store);
         i < std::get<double>(end->store); i += std::get<double>(step->store)) {
...
object *object::b_xor(object *o) {
  if (is_int(false) && o->is_int(false)) {
    object *ret = new object(o_num);
    ret->set((double)((int)std::get<double>(store) ^
                      (int)std::get<double>(o->store)));
    return ret;
  }
  err("^ not supported here");
  return new object();
  ...
  if (type == o->type) {
    ...
    case o_arr: {
      ...
      if (len() != o->len())
        ...
        for (int i = 0; i < std::get<std::vector<object *>>(store).size(); ++i)
          if (std::get<std::vector<object *>>(store)[i] !=
              std::get<std::vector<object *>>(o->store)[i])
            equals = false;
      }
      ret->set(equals);
      break;
...
void object::set(
    std::variant<double, std::string, bool, std::vector<object *>,
                 std::queue<object *>, std::stack<object *>,
                 std::unordered_set<object *, obj_hash, obj_equals>,
                 std::unordered_map<object *, object *, obj_hash, obj_equals>>
        _store) {
  store.swap(_store);
}